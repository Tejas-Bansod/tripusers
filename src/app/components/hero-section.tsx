...
export function HeroSection() {
  ...
  return (
    <section className="relative z-10 w-full h-[630px] md:h-[550px] lg:h-[650px] overflow-hidden group bg-black">
        ...
        <CarouselContent className="h-full ml-0">
          {slides.map((slide, index) => (
            <CarouselItem key={index} className="relative h-full pl-0">
              {/* Slide Content */}
              <div className="relative h-full w-full px-4 md:px-12 flex flex-col items-center justify-start pt-12 md:pt-16 md:justify-center text-center text-white pb-0 md:pb-115">
                <div className="max-w-5xl w-full space-y-1 md:space-y-6 animate-in fade-in slide-in-from-bottom-10 duration-1000">
      ...
      <div className="absolute z-20 inset-x-0 bottom-0 pb-2 md:pb-10 bg-gradient-to-t from-black/80 via-black/40 to-transparent pt-12 md:pt-20 pointer-events-none">
        <div className="container mx-auto px-2 md:px-0 space-y-2 md:space-y-8 pointer-events-auto">

          {/* Search Bar - Premium Glassmorphism */}
          <div className="bg-white/95 backdrop-blur-2xl rounded-2xl md:rounded-full p-1 md:p-1.5 shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col md:flex-row items-center gap-1 md:gap-1.5 max-w-4xl mx-auto border border-white transition-all duration-500">
            <div className="flex-1 w-full flex items-center px-3 py-1.5 md:px-4 md:py-1.5 gap-2 md:gap-3 border-b md:border-b-0 md:border-r border-slate-200/60">
              <div className="bg-red-50 p-1.5 md:p-2 rounded-full shrink-0">
                <MapPin className="h-4 w-4 md:h-4 md:w-4 text-red-600" />
              </div>
              <div className="flex flex-col flex-1">
                <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Destination</span>
                  <Input
                    placeholder="Where to?"
                    className="border-none focus-visible:ring-0 text-slate-900 placeholder:text-slate-400 h-6 md:h-6 p-0 text-sm md:text-base font-bold bg-transparent"
                  />
              </div>
            </div>

            <div className="flex-[1.2] w-full flex items-center px-3 py-1.5 md:px-4 md:py-1.5 gap-2 md:gap-3">
              <Popover>
                <PopoverTrigger asChild>
                  <button className="flex items-center gap-2 md:gap-3 w-full text-left focus:outline-none group/btn">
                    <div className="bg-slate-50 p-1.5 md:p-2 rounded-full group-hover/btn:bg-red-50 transition-colors shrink-0">
                      <Calendar className="h-4 w-4 md:h-4 md:w-4 text-slate-500 group-hover/btn:text-red-600 transition-colors" />
                    </div>
                    <div className="flex flex-col flex-1">
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <ChevronRight className={cn("h-4 w-4 md:h-4 md:w-4 text-slate-300 transition-transform group-hover/btn:text-red-400", "rotate-90")} />
                      </div>
                    </div>
                  </button>
                </PopoverTrigger>
            ...
            <Button className="w-full md:w-auto bg-red-600 hover:bg-red-700 text-white rounded-xl md:rounded-full px-8 py-2 md:py-4 h-auto text-sm md:text-sm font-black shadow-xl shadow-red-600/20 transition-all hover:scale-[1.02] md:hover:scale-105 active:scale-95 uppercase tracking-wider">
              <Search className="mr-2 h-4 w-4 md:h-4 md:w-4 stroke-[3]" />
              Find Tours
            </Button>
          </div>

          {/* Trust Bar - Integrated Pill Style */}
          <div className="flex flex-row flex-wrap justify-center items-center gap-x-2 md:gap-x-6 gap-y-1.5 py-1 md:py-2.5 px-3 md:px-6 bg-white/10 backdrop-blur-md rounded-xl md:rounded-full max-w-fit mx-auto border border-white/20">
          ...
          <div className="flex overflow-x-auto md:grid md:grid-cols-2 lg:grid-cols-4 gap-2 md:gap-3 max-w-5xl mx-auto px-4 md:px-0 py-1 md:py-5 items-stretch snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {categories.map((cat, i) => (
              <div
                key={i}
                className="bg-white/95 backdrop-blur-2xl rounded-2xl p-2.5 md:p-3 shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white relative overflow-hidden flex flex-col h-full min-w-[160px] md:min-w-0 snap-center shrink-0"
              >
